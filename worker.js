...
// Resolve the first data_source_id for a database (required by 2025-09-03 API)
// Result is cached in KV to avoid an extra API call on every sync
async function getDataSourceId(env) {
  const cacheKey = '__notion_data_source_id__';
  const cached = await env.NOTES_KV.get(cacheKey);
  ...
  if (!res.ok) {
    ...
    throw new Error(`Could not retrieve database: ${e.slice(0, 200)}`);
  }

  const data = await res.json();
  // 2025-09-03: retrieve database returns { data_sources: [{id, name}] }
  const dsId = data.data_sources?.[0]?.id || env.NOTION_DATABASE_ID;
  // Cache for 24 hours
  await env.NOTES_KV.put(cacheKey, dsId, { expirationTtl: 86400 });
  return dsId;
}

function noteToNotionBody(note, dataSourceId) {
  return {
    ...
    try {
      if (note.notionId) {
        ...
        const res = await fetch(`${NOTION_API}/pages/${note.notionId}`, {
          method: 'PATCH',
          headers: notionHeaders(env),
          body: JSON.stringify({ properties: noteToNotionBody(note, dsId).properties }),
        });
        if (res.ok) {
          results.updated++;
          continue;
      ...
      const res = await fetch(`${NOTION_API}/pages`, {
        method: 'POST',
        headers: notionHeaders(env),
        body: JSON.stringify(noteToNotionBody(note, dsId)),
      });
