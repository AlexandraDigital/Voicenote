// Colors and constants
    ...
    // NOTION SYNC CONFIG
    // ⚠️  Replace WORKER_URL with your deployed Cloudflare Worker URL
    const WORKER_URL = "https://voicenote-worker.YOUR-SUBDOMAIN.workers.dev";

    async function exportNotesToNotion() {
      try {
        showStatus("📤 Pushing notes to Notion...");
        const response = await fetch(`${WORKER_URL}/sync/push`, { method: 'POST' });
        if (response.ok) {
          const d = await response.json();
          showStatus(`✅ Synced to Notion! ${d.pushed} new, ${d.updated} updated.`);
        } else {
          const e = await response.json().catch(() => ({}));
          showStatus("❌ Push failed: " + (e.error || response.status));
        }
      } catch (err) {
        console.error("Push error:", err);
        showStatus("❌ Could not reach worker. Check WORKER_URL in app.js.");
      }
    }

    async function importNotesFromNotion() {
      try {
        showStatus("📥 Pulling notes from Notion...");
        const response = await fetch(`${WORKER_URL}/sync/pull`, { method: 'POST' });
        if (response.ok) {
          const d = await response.json();
          // Reload notes from worker after pull
          await loadNotesFromWorker();
          showStatus(`✅ Pulled from Notion! ${d.pulled} new/updated, ${d.skipped} unchanged.`);
        } else {
          const e = await response.json().catch(() => ({}));
          showStatus("❌ Pull failed: " + (e.error || response.status));
        }
      } catch (err) {
    ...
    async function loadNotesFromWorker() {
      try {
        const res = await fetch(`${WORKER_URL}/notes`);
        if (res.ok) {
          const workerNotes = await res.json();
          // Merge with local notes, worker version wins on conflict
          const merged = {};
          notes.forEach(n => { merged[n.id] = n; });
          workerNotes.forEach(n => { merged[n.id] = { ...n, date: new Date(n.updated || n.created) }; });
          notes = Object.values(merged);
          persistNotes();
          renderNotes();
        }
      } catch (_) { /* silent — use local notes as fallback */ }
    }
