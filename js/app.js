// Colors and constants
    ...
    function noteToWorkerFormat(note) {
      return {
        id: String(note.id),
        title: note.title || "Untitled",
        content: note.content || "",
        color: NOTE_COLORS[note.colorIdx ?? 0]?.bg || "#fff7ed",
        tags: note.tags || [],
        created: note.date ? new Date(note.date).toISOString() : new Date().toISOString(),
      };
    }

    function workerNoteToLocal(n) {
      const colorMap = {};
      NOTE_COLORS.forEach((c, i) => { colorMap[c.bg] = i; });
      return {
        id: n.id,
        title: n.title || "Untitled",
        content: n.content || "",
        date: new Date(n.updated || n.created || Date.now()),
        colorIdx: colorMap[n.color] ?? 2,
        tags: n.tags || [],
      };
    }
    ...
    async function exportNotesToNotion() {
      try {
        ...
        await pushLocalToWorker();
        const res = await fetch(`${WORKER_URL}/sync/push`, { method: "POST" });
        if (res.ok) {
    ...
    async function importNotesFromNotion() {
      try {
        ...
        if (notesRes.ok) {
          notes = (await notesRes.json()).map(workerNoteToLocal);
          persistNotes();
          renderNotes();
          showStatus(`✅ ${d.pulled} notes pulled from Notion!`);
        }
      } catch (err) {
    ...
    async function syncWithNotion() {
      try {
        showStatus("🔄 Syncing with Notion...");
        await pushLocalToWorker();
        const res = await fetch(`${WORKER_URL}/sync/auto`, { method: "POST" });
        if (res.ok) {
          const d = await res.json();
          const notesRes = await fetch(`${WORKER_URL}/notes`);
          if (notesRes.ok) {
